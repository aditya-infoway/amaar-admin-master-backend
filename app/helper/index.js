...
const categoryURL = (blobName) => {
  return blobName == "" || blobName == undefined || blobName == null
    ? ""
    : getBlobTempPublicUrl("icons/" + blobName);
};
...
const saveResizedImage = (path, filename) => {
  sharp(path)
    ...
    .toFormat("jpeg")
    .toFile(`./media/images/resized_${filename}`);
...
const selectWithJoinsV2 = async (
  ...
  // Build the JOIN clauses
  const joinString = joinClauses
    .map((joinClause) => {
      const { table, alias, onClause } = joinClause;
      const onString = Object.entries(onClause)
  ...
  // Build the WHERE clause
  const whereString = whereClauses
    ? `WHERE ${Object.entries(whereClauses)
        .map(([key, value]) => {
          if (typeof value === "object" && value !== null) {
            const operator = Object.keys(value)[0];
            const columnValue = value[operator];
  ...
  const orderByString =
    orderBy.length != 0
      ? `ORDER BY ${orderBy.map((column) => `${column[0]} ${column[1]}`).join(", ")}`
      : "";
...
const cartitemstotal = async (userId) => {
  ...
  var joinTables = [
    {
      table: "stock",
      alias: "sto",
      onClause: { "sto.stockId": { "=": "cart.productvariantId" } },
    },
    {
      table: "product",
      alias: "p",
      onClause: { "p.productId": { "=": "sto.productId" } },
    },
  ];
  var attributes = [
    "cart.cartId",
    ...
    "p.name",
    "sto.price",
    "sto.quantity AS stocks",
    "cart.quantity",
    "sto.combination",
    "(SELECT `image` FROM `image` WHERE `productId`=`sto`.`productId` ORDER BY `imageId` LIMIT 1)AS `image`",
  ];
  var order = [["cart.cartId", "DESC"]];

  var getproduct = await selectWithJoinsV2(
    tableName,
    joinTables,
    whereClause,
    attributes,
  ...
  for (var i = 0; i < getproduct.length; i++) {
    ...
    for (var j = 0; j < combinationarray.length; j++) {
      ...
      var varinatName = await selectWithJoins(
        "variant",
        [],
        { variantId: combinationarray[j][0] },
        ["name"],
        [],
        null,
        0,
...
const checkcouponfororder = async (couponId, alloverprice) => {
  ...
  var chkcoupon = await getResult(
    "SELECT `couponId`,`type`,`discount`,`maxDiscount` FROM `coupon` WHERE `couponId`='" +
      couponId +
      "' AND `startDate`<='" +
      today +
      "' AND `endDate`>='" +
  ...
  if (chkcoupon.length > 0) {
    if (chkcoupon[0]["type"] == "FLAT") {
      ...
      cpndata.push(
        chkcoupon[0]["couponId"],
        chkcoupon[0]["type"],
        totalAmount,
        chkcoupon[0]["discount"],
      );
...
const checkcouponfororderwithvendorold = async (
  couponId,
  alloverprice,
  userId,
) => {
  ...
  var chkcoupon = await getResult(
    "SELECT `couponId`,`type`,`discount`,`maxDiscount`,`vendorId` FROM `coupon` WHERE `couponId`='" +
      couponId +
      "' AND `startDate`<='" +
      today +
      "' AND `endDate`>='" +
  ...
  if (chkcoupon.length > 0) {
    ...
    if (chkcoupon[0]["vendorId"] > 0) {
      ...
      var getvenpro = await getResult(
        "SELECT `cart`.`quantity`,`stock`.`price` FROM `cart` LEFT JOIN `stock` ON `cart`.`productvariantId`=`stock`.`stockId` LEFT JOIN `product` ON `product`.`productId`=`stock`.`productId` WHERE `cart`.`userId`='" +
          userId +
          "' AND `product`.`vendorId`='" +
          chkcoupon[0]["vendorId"] +
          "'",
      );
      ...
      if (getvenpro.length > 0) {
        for (var i = 0; i < getvenpro.length; i++) {
          ...
          newprice =
            newprice +
            parseFloat(getvenpro[i]["price"]) *
              parseFloat(getvenpro[i]["quantity"]);
        }
      }
    ...
    if (chkcoupon[0]["type"] == "FLAT") {
      var totalAmount =
        alloverprice - newprice + (newprice - chkcoupon[0]["discount"]);
      cpndata.push(
        chkcoupon[0]["couponId"],
        chkcoupon[0]["type"],
        totalAmount,
...
const checkcouponfororderwithvendor = async (
  couponId,
  alloverprice,
  userId,
) => {
  ...
  // check coupon exist or not
  var chkcoupon = await getResult(
    "SELECT `couponId`,`type`,`discount`,`minOrderAmount`,`maxDiscount`,`vendorId` FROM `coupon` WHERE `couponId`='" +
      couponId +
      "' AND `startDate`<='" +
      today +
      "' AND `endDate`>='" +
...
const generateBookingCode = async () => {
  var code = "VB";
  var bookc = await selectWithJoins(
    "booking",
    [],
    { delete: 0 },
    ["bookingId"],
...
const getFCMToken = async (type = "USER", id) => {
  var fcm = await selectWithJoins(
    "fcmtoken",
    [],
    { delete: 0, recordId: id, recordType: type },
    ["token"],
    (orderby = [["fcmtokenId", "DESC"]]),
    1,
  );
...
const selectWithJoinsOrV2 = async (
  tableName,
  joinClauses,
  whereClauses,
  selectColumns,
