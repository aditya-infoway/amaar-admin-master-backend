...
module.exports = sequelize => {
  const attributes = {
    ...
    quantity: { type: DataTypes.STRING(20), allowNull: true, field: "quantity" },
    unit: { type: DataTypes.STRING(20), allowNull: true, field: "unit" },

    serialNo: { type: DataTypes.STRING(50), allowNull: true, field: "serialNo" },
    asslyQty: { type: DataTypes.STRING(20), allowNull: true, field: "asslyQty" },
    ...
    shapeDim: { type: DataTypes.STRING(50), allowNull: true, field: "shapeDim" },
    finQtty: { type: DataTypes.STRING(20), allowNull: true, field: "finQtty" },
    shape: { type: DataTypes.STRING(50), allowNull: true, field: "shape" },
    length: { type: DataTypes.STRING(20), allowNull: true, field: "length" },
    width: { type: DataTypes.STRING(20), allowNull: true, field: "width" },
  ...
  const options = {
    tableName: "bomitem",
    comment: "BOM tree items (self-referencing via parentId, item referenced by itemId only)",
    indexes: [
      { fields: ["bomId"] },
      { fields: ["parentId"] },
      { fields: ["itemId"] },
